...
type Detail = { session: { id: string; total: number; policy: { checkoutAttempt?: { items?: { productId: string; name: string; quantity: number; unitPrice: number }[] } }; cart: { productId: string; quantity: number }[] }; events: { id: string; title: string; detail: string }[] };
export default function OrdersPage() {
  ...
  async function reconcile(id: string) {
    setBusy(true); setError("");
    try { const response = await fetch("/api/orders/reconcile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: id }) }); const result = await response.json() as { error?: string }; if (!response.ok) throw new Error(result.error); await load(); await open(id); }
    catch (cause) { setError((cause as Error).message); } finally { setBusy(false); }
  }
  return <main className="catalogue-settings"><AccountMenu/><a href="/merchant">← Merchant overview</a><h1>Orders & checkout review</h1><p>Order creation is separate from payment confirmation. Unconfirmed checkouts keep their stock reserved.</p><button disabled={busy} onClick={() => void load()}>Refresh orders</button>{error && <p role="alert">{error}</p>}{!busy && !orders.length && <p>No checkouts yet. Approved shopping carts appear here.</p>}<div className="catalogue-editor">{orders.map(order => <section className="account-card" key={order.id}><h2>{order.title}</h2><p>{order.status === "ordered" ? "Order created" : order.status === "cancelled" ? "Cancelled · stock released" : order.attempt?.state === "unknown" ? "Needs review · stock reserved" : "Pending · stock reserved"}</p><strong>₹{(order.total / 100).toLocaleString("en-IN")}</strong><p>{order.id}</p><p>{order.orderId ?? "No confirmed provider order"}</p><button onClick={() => void open(order.id)}>View details</button>{order.status === "approved" && <button disabled={busy} onClick={() => void reconcile(order.id)}>Check outcome & recover</button>}</section>)}</div><p><button disabled={offset === 0 || busy} onClick={() => setOffset(offset - 50)}>Previous</button> <button disabled={!more || busy} onClick={() => setOffset(offset + 50)}>Next</button></p>{detail && <section className="account-card"><h2>Checkout details</h2><p>{detail.session.id}</p><ul>{detail.session.policy.checkoutAttempt?.items ? detail.session.policy.checkoutAttempt.items.map(item => <li key={item.productId}>{item.name} × {item.quantity} · ₹{(item.unitPrice * item.quantity / 100).toLocaleString("en-IN")}</li>) : detail.session.cart.map(item => <li key={item.productId}>{item.productId} × {item.quantity} (legacy item record)</li>)}</ul><p>Approved amount: ₹{(detail.session.total / 100).toLocaleString("en-IN")}</p><a href={`/audit?sessionId=${encodeURIComponent(detail.session.id)}`}>Open full audit trail</a>{detail.events.map(event => <div key={event.id}><h3>{event.title}</h3><p>{event.detail}</p></div>)}</section>}</main>;
}
