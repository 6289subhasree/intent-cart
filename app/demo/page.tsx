...
type Policy = {
  withinBudget: boolean; stockValid: boolean; deliveryValid: boolean; quantitiesValid: boolean; unavailableProductIds?: string[]; passed: boolean; violations: string[];
};
...
export default function DemoPage() {
  ...
  const [buildFailure, setBuildFailure] = useState<{ title: string; detail: string } | null>(null);
  ...
  const inventoryFailure = unavailableNames.length > 0;
  const checkoutLocked = session?.status === "approved" || session?.status === "ordered";
  ...
  async function postJson<T = SessionPayload>(url: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      if (code === "ORDER_REVIEW_REQUIRED" || code === "CHECKOUT_LOCKED") setSession((current) => current ? { ...current, status: "approved" } : current);
      throw Object.assign(new Error(result.error ?? "That action could not be completed."), { code });
    }
    return result;
  }
  ...
  async function buildCart() {
    ...
    setSession(null); setProducts([]); setCheckoutOpen(false); setBuildFailure(null);
    setStage("thinking"); setError(null); setPaid(false); setRepaired(false);
    try {
      applySession(await postJson("/api/agent", { intent: request }));
    } catch (cause) {
      const detail = cause instanceof Error ? cause.message : "The cart could not be built.";
      const code = (cause as { code?: string }).code;
      setBuildFailure({ title: code === "NO_MATCH" ? "No products match your request" : code === "CLARIFICATION_REQUIRED" ? "A little more detail, please" : "We couldn’t build your cart", detail });
      setStage("ready"); setError(detail);
    }
  }

  async function changeCart(action: "update" | "conflict" | "repair", nextProducts = products) {
    if (!session || checkoutLocked || draftChanged || busyAction || stage === "thinking") return;
    ...
    <main className="site-shell">
      ...
      <section className="workspace" id="shop">
        <aside className="intent-panel">
          ...
          <button className="failure-trigger" disabled={checkoutLocked || draftChanged || !session || busyAction !== null} onClick={() => void changeCart("conflict")}><TriangleAlert />Trigger inventory conflict</button>
        ...
        <section className="cart-panel" aria-live="polite">
          {stage === "thinking" ? (
          ...
          ) : buildFailure ? <div className="thinking-state" role="status"><span className="thinking-orbit"><Search /></span><h2>{buildFailure.title}</h2><p>{buildFailure.detail}</p><p>Your budget: {money(budget)}. Checkout is unavailable until a matching cart is built.</p></div> : <>
            ...
            {session && error && <div className="inline-error" role="alert"><TriangleAlert />{error}</div>}
        ...
        <aside className="order-panel">
          ...
          {session && <div className="delivery-card"><PackageCheck /><div><strong>{deliveryEstimate}</strong><p>Catalogue estimate; arrival date is not guaranteed</p></div><CheckCircle2 /></div>}
          ...
          <Button className="checkout-button" onClick={() => { setPaid(false); setError(null); setCheckoutOpen(true); }} disabled={checkoutLocked || draftChanged || !session || !session.policy.passed || session.status === "ordered" || busyAction !== null}>Review & approve <ArrowRight /></Button>
          {session?.status === "approved" && <p role="status">Checkout is locked while its outcome is confirmed. <a href={auditHref}>View the saved audit trail</a>.</p>}
